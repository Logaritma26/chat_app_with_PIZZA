...
public class HomeFireAdapter extends FirestoreRecyclerAdapter<HomeFireChats, HomeFireAdapter.ViewHolder> {
    ...
    public HomeFireAdapter(@NonNull FirestoreRecyclerOptions<HomeFireChats> options, Recycler_Click recycler_click, Context context) {
        super(options);
    ...
    protected void onBindViewHolder(@NonNull ViewHolder holder, int position, @NonNull HomeFireChats model) {
        ...
        if (model.getPp_url().equals("")) {
            holder.pp.setImageResource(R.drawable.default_pp);
        } else {
            Glide
                    .with(context)
                    .load(model.getPp_url())
                    .centerCrop()
                    .into(holder.pp);
        }
